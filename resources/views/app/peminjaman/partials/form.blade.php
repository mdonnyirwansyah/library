<div class="form-group row mb-4">
    ...
    <div class="col-sm-12 col-md-7">
        <input type="text" class="form-control" name="nis" id="nis" @isset($peminjaman) value="{{ $anggota->nis }}" @endisset />
...
<div class="form-group row mb-4">
    ...
    <div class="col-sm-12 col-md-7">
        <select class="form-select select2" style="width: 100%" name="buku[]" id="buku" multiple>
          ...
          <option value="{{ $item->id }}" @isset($peminjaman) @if(in_array($item->id, $peminjaman->buku->pluck('id')->toArray())) selected @endif @endisset>{{ $item->kode.' - '.$item->judul }}</option>
          @endforeach
        </select>
